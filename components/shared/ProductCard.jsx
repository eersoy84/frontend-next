...
export default function ProductCard({ product }) {
  ...
  const [updatedParticipants, setUpdatedParticipants] = useState(
    (product && product.participants) || 0
  );
  ...
  const image = product.imageUrl ? (
    <Link href={`/ilanlar/${product.adId}?seoUrl=${friendlyUrl}`}>
      <a style={{ backgroundImage: `url(${product?.imageUrl})` }}></a>
    </Link>
  ) : (
    <Spinner />
  );
  ...
  if (product.instantPrice) {
    ...
    instantProfit = `${(
      product.productPrice - product.instantPrice
  ...
  if (product.instantDiscountPercent) {
    instantDiscount = `% ${product.instantDiscountPercent.toLocaleString(
      undefined,
      {
        minimumFractionDigits: 2,
        maximumFractionDigits: 2,
  ...
  return (
    <>
      <div style={{ display: "block" }}>
        <div
          className="card p-3 bizleal_product_list_container2"
            ...
            <div className="col-10 col-sm-4">
              <Link href={`/ilanlar/${product.adId}?seoUrl=${friendlyUrl}`}>
                <a>
                  ...
                  <div className="bizleal_product_list__title--second">
                    {product.description}
                  </div>
                </a>
              </Link>
              ...
              <div className="row mt-3 mt-sm-0">
                <div className="col-12 d-flex justify-content-between mb-3">
                  {product.numOrders > 0 && (
                    ...
                    >
                      Satılan:&nbsp;
                      <span className="text-success">{product.numOrders}</span>
                    </div>
                  )}
                  {product.numOrders > 0 && (
                    <div
                ...
                >
                  ...
                  >
                    İndirim:&nbsp;&nbsp;
                    <span className="text-success">{instantDiscount}</span>
                  </span>
                </div>
                {hasMounted && (
                  <ToolTip
                ...
                >
                  ...
                  <span
                    ...
                    className="pl-2 d-flex align-items-baseline"
                  >
                    Son fiyat:&nbsp;<span>{targetPrice}</span>
                  </span>
                </div>
                ...
                >
                  ...
                  <span
                    style={{ fontSize: "12px" }}
                    className="pl-2 d-flex align-items-baseline text-info"
                  >
              ...
              {navigator?.share ? (
                <span>
                  <button
                    ...
                    onClick={() => {
                      shareSheet(
                        `https://bizleal.com/ilanlar/${product.adId}?seoUrl=${friendlyUrl}`
                      );
                    }}
                  >
                    <svg
