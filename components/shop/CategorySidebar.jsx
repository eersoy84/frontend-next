...
export default function CategorySidebar(props) {
  ...
  const { sidebarState, categories } = useSelector((state) => ({
...
const dispatch = useDispatch();
  ...
  }, [offcanvas, sidebarState.open, dispatch(sidebarClose)]);
  ...
  return (
    <div className={classes}>
      ...
      <div className="block-sidebar__backdrop" ref={backdropRef} onClick={() => dispatch(sidebarClose())} />
      <div className="block-sidebar__body" ref={bodyRef}>
        <div className="block-sidebar__header">
          <div className="block-sidebar__title">Filtreler</div>
          <button className="block-sidebar__close" type="button" onClick={() => dispatch(sidebarClose())}>
            <Image src="/icons/cross-20.svg" height={20} width={20} />
            
          </button>
        </div>
        <div className="block-sidebar__item">
          <FilterWrapper categories={categories} offcanvas={offcanvas} />
        </div>
        {offcanvas !== 'always' && (
